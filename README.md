...
工程處理重點
CSV 相容性：自動處理 UTF-8 BOM、不間斷空白與舊檔名。
資料型別：移除價格中的 `$` 與千分位，再轉為整數。
欄位穩定性：API 固定輸出五個欄位，避免額外 index 欄造成顯示錯位。
輸入安全：搜尋使用純文字比對，前端輸出進行 HTML escape。
可觀測性：`/api/health` 回報服務狀態與餐點筆數。
